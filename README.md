...
# Читать логи из Docker-контейнера
./logspector -docker my-app-container

# Читать логи из Kubernetes pod'а в другом namespace
./logspector -kube my-app-pod -kube-args "-n staging"
...
| `-docker`   | Имя или ID Docker-контейнера для чтения логов                |
| `-f`        | Путь к файлам с логами (можно указать несколько через запятую) |
| `-h`        | Показать справку                                             |
| `-json`     | Принудительный pretty print для JSON                         |
| `-kube-args`| Дополнительные аргументы для kubectl (например, "-n my-ns")  |
